...
const Partners = () => {
    const { isFetching, error, partners, onGetPricingPartners, onUpdatePricingPartners } = useContentPricing()
    ...
    const handleUpdatePricingPartners = async(values: ArticleData[]) => {
        const response = await onUpdatePricingPartners(values)
        if (!response) return
        await onGetPricingPartners()
        return response
    }
