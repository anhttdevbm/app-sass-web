...
type SelectTagProps = {
  ...
  label?: string;
  indexArray: number;
  onEnter?: (value: string | undefined, indexArray: number) => void;
  onSelect: (event: React.ChangeEvent<{}>, value: any, indexArray: number) => void;
  handleDelete: (value: any, indexArray) => void;
  ...
  const renderTags = (value: TagData[], getTagProps: any) => {
    ...
    return tagIds.map((tagId, index) => {
      ...
      const onDelete = (label) => {        
        const newValue = value.filter((tag) => tag.tag !== label);
        handleDelete(newValue, indexArray)
      }
      ...
      return (
        <Chip
          ...
          // onDelete={() => {
          //   handleDelete(value, indexArray as number)
          // }}
          onDelete={(e) => 
            onDelete(tag.tag)
          }
      ...
      renderInput={(params) => (
        <Input
          ...
          onKeyDown={(e) => {
            if (e.key === "Enter") {
              e.preventDefault();
              onEnter && onEnter(inputRef.current?.value, indexArray);
            }
          }}
          {...params}
          title={label}
