...
const Banner1 = () => {
    const { isFetching, error, pricingBanner : banner, onGetPricingBanner, onUpdatePricingBanner } = useContentPricing()
    ...
    const handleUpdatePricingBanner = async(values: ContentData) => {
        const response = await onUpdatePricingBanner(values)
        if (!response) return
        await onGetPricingBanner()
        return response
    }
