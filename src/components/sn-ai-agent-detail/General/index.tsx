...
import {
  SelectChangeEvent,
  Stack,
  Theme as MuiMaterialTheme,
} from "@mui/material";
import { Theme as MuiSystemTheme } from "@mui/system";
...
export const General = () => {
  ...
  const handleUpdateAgent = useCallback(async () => {
    ...
    let isChanged = Object.keys(newAgentData).some(
      (key) =>
        JSON.stringify(newAgentData[key as keyof UpdateAIAgentPayload]) !==
        JSON.stringify(aiAgent[key as keyof UpdateAIAgentPayload]),
    );
    ...
    if (
      (image !== ImgPlaceHolderAgent.src || image !== aiAgent.avatar) &&
      file
    ) {
    ...
    <>
      <Stack flex={1} direction={"column"} spacing={3} padding={4}>
        ...
        <Select
          ...
          value={tone}
          theme={theme as unknown as MuiMaterialTheme & MuiSystemTheme}
          options={toneOptions}
