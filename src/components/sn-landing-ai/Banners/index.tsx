...
const Banners = () => {
    const { isFetching, error, aiBanner : banner, onGetAIBanner, onUpdateAIBanner } = useContentAI()
    ...
    const desktopHeaderList: CellProps[] = useMemo(
        () => [
            ...
            { value: contentT("tableList.image"), width: "20%", align: "left" },
    ...
    const handleUpdateAIBanner = async(values: ContentData) => {
        const response = await onUpdateAIBanner(values)
        if (!response) return
        await onGetAIBanner()
        return response
    }
