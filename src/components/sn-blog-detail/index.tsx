...
const BlogDetailSection = () => {
  ...
  useEffect(() => {
    if (
      Array.isArray(detailItem?.attachments_down) &&
      detailItem?.attachments_down
    ) {
      const filesArray: Promise<File>[] = detailItem?.attachments_down.map(
        async (attachment) => {
                ...
                <Grid item xs={12} sm={8} padding={6}>
                  <Paper
                    sx={{
                      ...
                      backgroundImage: `url(${
                        detailItem?.background_down?.link || ""
                      })`,
                    }}
                  >
                    <Box
              ...
              initialValues={
                {
                  ...
                  attachments: detailItem?.attachments_down?.map(
                    (a) => a.object,
                  ),
...
const renderContentWithAttachments = (
  content: string,
  attachments: AttachmentsBlogs[],
) => {
  if (typeof content !== "string") {
    return null;
  }

  const attachmentPlaceholders = content
    .split(/<\/p>|<br\s*\/?>/)
    ...
    <div className="blog-post">
      {attachmentPlaceholders.map((placeholder, index) => (
        <React.Fragment key={index}>
          <div
            className="content"
            dangerouslySetInnerHTML={{ __html: parseHTML(placeholder) }}
          />
      ...
      {attachments &&
        attachments
          .slice(attachmentPlaceholders.length)
