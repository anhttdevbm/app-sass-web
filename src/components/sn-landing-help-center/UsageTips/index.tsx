...
import { useContentAboutUs, useContentCenter } from "store/content/selectors"
...
const UsageTips = () => {
    const { isFetching, error, usageTips, onGetHelpCenterUsageTips, onUpdateHelpCenterUsageTips } = useContentCenter()
    ...
    const desktopHeaderList: CellProps[] = useMemo(
        () => [
          ...
          { value: contentT("tableList.link"), width: "20%", align: "left" },
          { value: contentT("tableList.image"), width: "40%", align: "center" },
        ],
        [contentT],
    );
    ...
    const handleUpdateHelpCenterUsageTips = async(values: ContentData[]) => {
        const response = await onUpdateHelpCenterUsageTips(values)
        if (!response) return
        await onGetHelpCenterUsageTips()
        return response
    }
        ...
        <>
            <StatusServer isFetching={isFetching} error={error}>
                ...
                >
                    ...
                    >
                        <Text variant="h4">
                            {contentT("helpCenter.usage_tips")}
                        </Text>
                        <IconButton
            ...
            {action === DataAction.UPDATE && (
                <Form
                    ...
                    initialValues={usageTips}
                    onSubmit={handleUpdateHelpCenterUsageTips}
                    titleForm={contentT("helpCenter.update_usage_tips")}
                />
            )}
        </>
    )
