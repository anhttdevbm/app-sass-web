...
const Banners = () => {
    const { isFetching, error, centerBanner : banner, onGetHelpCenterBanner, onUpdateHelpCenterBanner } = useContentCenter()
    const contentT = useTranslations(NS_CONTENTS);
    const [action, setAction] = useState<DataAction | undefined>();

    const desktopHeaderList: CellProps[] = useMemo(
        () => [
          { value: contentT("tableList.description"), width: "70%", align: "left" },
    ...
    const onHandleUpdateBanner = async(values: BannerCenterData) => {
        const response = await onUpdateHelpCenterBanner(values)
        if (!response) return
        await onGetHelpCenterBanner()
        return response
    }
