...
type FormProps = {
  initialValues: BannerFormAboutUs[];
  type: DataAction;
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  onSubmit: (values: BannerFormAboutUs[]) => Promise<any>;
  titleForm: string;
...
const Form = (props: FormProps) => {
  ...
  const onSubmit = async (values: BannerFormAboutUs[]) => {
    try {
              ...
              <Grid
                ...
                <Grid item xs={12} md={6}>
                  <UploadFile
                    ...
                    name="bannerUpload"
                    value={formik.values[index]?.bannerUpload ? formik.values[index]?.bannerUpload : formik.values[index]?.link}
                    index={index}
